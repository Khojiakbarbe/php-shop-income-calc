...
$pdoStatementGet = $pdo->prepare("SELECT * FROM `xodimlar` WHERE `user_id`=$id");
...
if (isset($_POST['submit'])) {
    ...
    $pdoPost = $pdo->prepare("
        UPDATE `xodimlar` SET 
        `olingan_qarz`=:olingan_qarz,`berilgan_qarz`=:berilgan_qarz,`comment`=:comment WHERE id=$id
    ");

    $pdoPost->bindParam("olingan_qarz", $olindi);
    $pdoPost->bindParam("berilgan_qarz", $berildi);
...
<body>
    ...
    <div class="container mt-5">
        <form action="xodimEdit.php?id=<?= $data['id'] ?>" method="post">
            <div class="row">
                ...
                <div class="col-6 mb-2">
                    ...
                    <input type="number" required="" name="olindi" id="price" value="<?= $data['olingan_qarz'] ?>"
                        class="form-control" placeholder="Olindi">
                </div>
                <div class="col-6 mb-2">
                    <label for="terminal">Berildi:</label>
                    <input type="number" required="" name="berildi" id="terminal" value="<?= $data['berilgan_qarz'] ?>"
